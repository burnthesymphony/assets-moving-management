<?php if($is_xls=='YA'){
	echo '<table>
			<tr><td><h1>Laporan Rekapitulasi Berdasarkan Departemen Periode '.bulan(str_pad($bulan,2, "0", STR_PAD_LEFT)).' '.$tahun.'</h1></td></tr>
...
<?php 
	if($is_xls=='YA'){
		$file="REKAP-PER-DEPARTEMEN-".bulan(str_pad($bulan,2, "0", STR_PAD_LEFT)).' '.$tahun.".xls";
		header("Content-type: application/vnd.ms-excel");
		header("Content-Disposition: attachment; filename=$file");
	}
	else{ ?>
		<a value="export xls" class="btn btn-primary" href="<?php echo base_url('laporan/rekap_departemen_xls/'.$bulan.'/'.$tahun.'/'.$nama_komponen_gaji) ?>" target="_blank" >Eksport XLS</a>		
	<?php } ?>