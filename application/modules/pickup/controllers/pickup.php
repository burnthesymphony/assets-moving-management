...
class Pickup extends MX_Controller {
    ...
    function verifikasi_cetak_sj($id_pickup){
         ...
         $data['form']  =array('no_surat_jalan' => array('name'=>'permit_number','id'=>'permit_number','class'=>'form-control input-sm'),
                               'loading_point'   => array('name'=>'loading_point','id'=>'loading_point','class'=>'form-control input-sm'),
                               'id_gate_to'     => array('name'=>'id_gate_to','id'=>'id_gate_to','class'=>'form-control input-sm'),
                               'car_number'     => array('name'=>'car_number','id'=>'car_number','class'=>'required form-control input-sm','title'=>'Nomor Kendaraan'),
        ...
        //echo $this->db->last_query();
         $data['result_detail'] =  $this->global_model->get_data(array('table'=>'trs_pickup_detail as a',
                                                                     'join'=>array('table'=>array('mst_item as b','mst_item_category  c'),
                                                                        ...
                                                                        'where'=>array('a.id_pickup'=>$id_pickup),
                                                                        'select'=>'a.*,b.item_name,b.id_item_category,b.description,b.barcode ,c.category_name,b.pic_department,b.barcode_old',
                                    'order_by'=>array('field'=>'item_name','type'=>'ASC'),
                                                                    )
                                                              );
    ...
    function verifikasi_benar_surat_jalan(){
      ...
      ## CREATE NO SURAT JALAN 
       $loading_point  = $this->global_model->get_data(array('table'=>'mst_gate_inventory','where'=>array('id_gate'=>$_POST['loading_point']),'data'=>'row'));
        $gate           = $this->global_model->get_data(array('table'=>'mst_gate_inventory','where'=>array('id_gate'=>$_POST['id_gate_to']),'data'=>'row'));
        $tahun          = date('Y');
        $bulan          = $this->_romawi(date('m'));
        $sql_max        = "SELECT max(no_urut) as last_no_urut  FROM trs_pickup WHERE status='TERBIT' ";
        $result_max     = $this->db->query($sql_max)->row_array() ;
        if(empty($result_max['last_no_urut']))
          $new_urut= 1;
        else
          $new_urut = $result_max['last_no_urut'] + 1;

        $no_surat_jalan =str_pad($new_urut, 3, "0", STR_PAD_LEFT).'-'.$loading_point['gate_name'].'-'.$gate['gate_name'].'-'.$bulan.'-'.$tahun;

      ## ENND CREATE NO SURAT JALAN
      ...
      $this->db->set('no_urut',$new_urut);
      $this->db->set('permit_number',$no_surat_jalan);
      $this->db->set('status','TERBIT');
      $this->db->where('id_pickup',$_POST['id_pickup']);

      $result= $this->db->update('trs_pickup');
      if(!empty($_POST['isi_box_item'])){
          foreach ($_POST['isi_box_item'] as $key => $value) {
             foreach ($_POST['isi_box_item'][$key] as $val) {
                $this->db->set('parent_item',$key);
                $this->db->set('id_pickup',$_POST['id_pickup']);
                $this->db->set('id_item',$val);
                $this->db->insert('trs_pickup_detail');
             }
             
          }
      }
      //$this->db->query("call InsertLogItem(3,".$data['result_item']['id_item'].",1,".$this->session->userdata('id_user').",1,".$data['last_id_pickup_detail'].")");
      echo $this->db->_error_message();
    } 
    ...
    function cetak_surat_jalan($id_pickup){
        ...
        $sql_detail="SELECT `a`.*, `b`.`item_name`, `b`.`id_item_category`, `b`.`description`, `b`.`barcode`, `b`.`barcode_old`, `c`.`category_name` 
                        FROM (`trs_pickup_detail` as a) LEFT JOIN `mst_item` as b ON `a`.`id_item`=`b`.`id_item` 
                          ...
                          WHERE `a`.`id_pickup` = '".$id_pickup."' AND `a`.`parent_item` is NULL ORDER BY `item_name` ASC";
